...
contract autocompounder is Ownable, ReentrancyGuard {
    ...
    function addAddress(address _address) external onlyOwner nonReentrant {
        ...
        pid[length] = PoolInfo(_address, 0, true, 0, 0, "", 0);
        poolInfo.push(pid[length]);
    }

    function harvestAllx2(uint _pid) external nonReentrant {
        IStrategy strategy = IStrategy(pid[_pid].strat);
        if( strategy.balanceOf() > 0 && strategy.rewardsAvailable() > 0 ){
            try strategy.harvest(){
                poolInfo[_pid].lastTimeHarvest = block.timestamp;
            }catch Error(string memory reason){
                poolInfo[_pid].error = reason;
            }catch{
                poolInfo[_pid].errors++;
            }
        }
    }
    ...
    function getInformation(uint _pid) view external
    returns (address, uint256, bool, string memory, uint256){
        return (
        poolInfo[_pid].strat,
        poolInfo[_pid].lastTimeHarvest,
        poolInfo[_pid].active,
        poolInfo[_pid].error,
        poolInfo[_pid].errors);
    }
}
